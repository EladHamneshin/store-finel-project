...
const updateAmount = async (userId: string, item: CartItem) => {
    const cart = await cartDal.getCartProducts(userId);
    if (!cart) throw new RequestError('No cart found', STATUS_CODES.NO_CONTENT);

    const prodInCart = cart.items.find((prod: any) => prod.product_id.toString() === item.product_id.toString());
    if (!prodInCart) {
...
const updateCart = async (userId: string, item:any) => {
    const dbCart = await cartDal.getCartProducts(userId);
    if (!dbCart)
        throw new RequestError('Cart not found', STATUS_CODES.NO_CONTENT);

    const index = dbCart.items.findIndex(
        (dbItem: any) => dbItem.product_id.toString() === item.product_id.toString()
    );
